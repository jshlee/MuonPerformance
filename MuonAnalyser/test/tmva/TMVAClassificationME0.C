...
int TMVAClassificationME0( TString myMethodList = "" )
{
   ...
   TString fname = "/xrootd/store/user/tt8888tt/muon/9_3_0_pre4/tmva/tenmu.root";
   ...
   TMVA::Factory *factory = new TMVA::Factory( "TMVAClassificationME0", outputFile,
   ...
   dataloader->AddVariable("muon_ME0deltaX", 'F');
   dataloader->AddVariable("muon_ME0deltaY", 'F');
   dataloader->AddVariable("muon_ME0pullX", 'F');
   dataloader->AddVariable("muon_ME0pullY", 'F');
   //dataloader->AddVariable("muon_ME0dPhiBend", 'F');
   dataloader->AddVariable("muon_ME0dPhi", 'F');
   dataloader->AddVariable("muon_ME0dEta", 'F');
   //dataloader->AddVariable("muon_ME0pullPhi", 'F');
   dataloader->AddVariable("muon_ME0deltaDXDZ", 'F');
   dataloader->AddVariable("muon_ME0deltaDYDZ", 'F');
   ...
   TCut mycuts =  "muon_signal && muon_isME0Muon && abs(muon.Eta())>2 && abs(muon.Eta())<2.8"; // for example: TCut mycuts = "abs(var1)<0.5 && abs(var2-0.5)<1";
   TCut mycutb = "!muon_signal && muon_isME0Muon && abs(muon.Eta())>2 && abs(muon.Eta())<2.8"; // for example: TCut mycutb = "abs(var1)<0.5";
   ...
   dataloader->PrepareTrainingAndTestTree( mycuts, mycutb,
                                        //"nTrain_Signal=200000:nTrain_Background=60000:SplitMode=Random:NormMode=NumEvents:!V" );//total
                                        "nTrain_Signal=70000:nTrain_Background=900000:SplitMode=Random:NormMode=NumEvents:!V" );//tenmu
                                        //"nTrain_Signal=10000:nTrain_Background=19000:SplitMode=Random:NormMode=NumEvents:!V" );//ttbar
   ...
   if (Use["BDT"])  // Adaptive Boost
      factory->BookMethod( dataloader, TMVA::Types::kBDT, "BDT",
                           "!H:!V:NTrees=1000:MinNodeSize=2.5%:MaxDepth=30:BoostType=AdaBoost:AdaBoostBeta=0.5:UseBaggedBoost:BaggedSampleFraction=0.5:SeparationType=GiniIndex:nCuts=20" );
...
int main( int argc, char** argv )
{
   ...
   return TMVAClassificationME0(methodList);
}
